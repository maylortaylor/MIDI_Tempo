...
bool  allColorStep = true;
bool  allColorFlash = true;
...
#define PWM_MaxPixels 8   //Number of Pixels
...
/***************************************************************************************
   MIDIBEAT Function: every midi beat coming from computer/ableton triggers this function
***************************************************************************************/
void midiBeat() {
	
	for (int i = 0; i < PWM_MaxPixels; i++) {
		if (allColorStep == true) {
			setPixel(i, 0, 50, 50);	
		}
		if (allColorStep == false) {
			setPixel(i, 50, 0, 50);
		}		
	}
	...
	if (measureStep > 7){
		fullMeasure++;
		measureStep = 0;
		allColorStep = !allColorStep;
	}
	
	
	// TRAVELING NOTE
	setPixel(measureStep, 150, 0, 0);
	
	if (measureStep == 0) {
		// DOWN BEAT FLASH
		if (allColorFlash == true) {
			setPixel(ALL, 255, 255, 255);			
		}
	}

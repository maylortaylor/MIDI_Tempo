...
Version 0.1 - BRS
  6/7/16 Forking code from "Ultime 6 Star System"
  6/8/16 Overhauled fade code to implement RGB
...
#include <MIDI.h>                 //MIDI Communication
...
uint8_t  downbeattwo = 4;
uint8_t  downbeatthree = 8;
uint8_t  downbeatfour = 12;
...
uint8_t fillerBeatColor[] = {138,11,176};
...
void setup() {
	...
	//USB MIDI setup for Hairless MIDI Bridge. Increases baud rate. 
	Serial.begin(115200);

	setPixelFade(ALL, 0, 0);

	//Read brightness from potentiometer
	int potentiometerKnob = analogRead(A4);
...
void loop()
{
	...
	if ((digitalRead(A1) == 0)&&(debounce == 0))
	{
		button ++;
	...
	switch(button)
	{
		case 0:
		{
			...
			fillerBeatColor[R] = 0;
			fillerBeatColor[G] = 0;
			fillerBeatColor[B] = 10;
			
			//Color of the LED at the full measure meteronome (past)
			oldColorsForMetronome[R] = downBeatColorR;
			oldColorsForMetronome[G] = downBeatColor[1];
			oldColorsForMetronome[B] = downBeatColor[2];
			
			//Color of the LED at the full measure meteronome (present)
			newColorsForMetronome[R] = 255;
			newColorsForMetronome[G] = 0;
